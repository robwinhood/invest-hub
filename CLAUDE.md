...
- ⑤ PR: **`/ship` 실행 한 번으로 자가검토 → `check-all` → 커밋 → 푸시 → PR 생성까지 자동화**된다. 머지는 기본 사람이 하며, 프롬프트에 "머지까지"를 명시하면 CI 통과 확인 후 자동 머지한다. (상세·예시: 아래 "자동화 Skill — PR까지 한 번에 (/ship)")
...
투자 대시보드에 새 섹션을 추가할 때는 수동으로 8개 파일을 만들 필요 없다.
...
## 자동화 Skill — PR까지 한 번에 (`/ship`)

개발이 끝난 피처 브랜치에서 `/ship`을 실행하면 **워크플로우 ④~⑤(자가검토 → PR 생성)** 를 한 번에 수행한다.
순서: `self-review` → `check-all` → 커밋 → 푸시 → `gh pr create`. PR 본문의 "AI 자동 단계 확인(①~④)"과 자가 검토 보고서까지 자동으로 채운다.

**Skill 정의**: `.claude/commands/ship.md`

```
# 1) 인자 없이 — 변경 내용에서 제목·본문 자동 작성 (가장 흔함)
/ship

# 2) PR 제목/요약 힌트를 줄 때
/ship 추천 캐시 L2에 TTL 지터 추가해 캐시 스탬피드 완화

# 3) 관련 이슈/PRD를 엮을 때
/ship #42 국내 ETF 섹션 추가 — PRD는 docs/prd-etf.md
...
**가드레일** (어기면 중단):
- `main`/`develop` 등 보호 브랜치 위에서 실행하면 중단하고 피처 브랜치를 따라고 안내한다.
