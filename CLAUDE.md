...
- **완전 자동 CI**: `.github/workflows/release.yml` 이 **main 머지 시** 위 스크립트를 돌려 버전 컷·`chore(release): vX.Y.Z` 커밋·`vX.Y.Z` 태그 무인 push + **GitHub Release 발행**(CHANGELOG `[X.Y.Z]` 섹션을 릴리즈 노트로)까지 한다. `[Unreleased]`가 비어 있으면 아무 일도 안 한다. 봇 커밋은 `if: !startsWith(... 'chore(release):')`로 재트리거 루프를 막는다.
