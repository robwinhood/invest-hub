...
> **`[Unreleased]` 작성 규칙** — 다음 릴리즈에 누적되는 변경을 적는 영역이다. **변경이 생긴 분류만** `### Added`/`### Changed`/`### Fixed` 하위 절로 두고, 빈 하위 절은 남기지 않는다(헤딩만 덩그러니 있으면 "작성하다 만 것"처럼 보이므로). 이 블록이 비어 있으면 릴리즈는 컷되지 않는다.
...
- README 「(3) 성능 최적화」·「(4) 신뢰성 검증」 표를 3열 → 2열로 슬림화 — 좁은 창에서 가로 스크롤로 깨지던 문제 해소. 부차 열(근거 수치 / 검증 테스트)을 본문 아래 `<br><sub>`로 접어, 긴 설정 문자열·테스트 클래스명 같은 줄바꿈 불가 토큰이 표 폭을 늘리지 않게 했다. R1~R8 표와 같은 하우스 스타일(굵은 펀치라인 + `<sub>` 보조정보)로 통일, 수치·테스트명 등 정보는 전부 보존.
...
- README '잠재적 위험 분석'(R1~R8) 표 가독성 개선: 덩그러니 있던 `#` 열을 리스크명에 합쳐 3열 → 2열로 줄이고, 대책을 설명하는 (2) 섹션의 `R1 · 연쇄 장애` 표기와 통일. 각 위험 설명을 결과가 굵게 먼저 읽히는 펀치라인 형태로 정리(정보 보존).
...
- CHANGELOG `[Unreleased]`가 빈 `Added/Changed/Fixed` 하위 절 탓에 "작성하다 만 것"처럼 보이던 문제 정리 — 변경이 있는 분류만 표기하고 빈 하위 절은 남기지 않는 규칙으로 바꿨다. 작성 규칙 안내문은 (릴리즈 컷에 딸려 내려가지 않도록) 파일 상단에 한 번만 두고, `scripts/release.sh`의 컷도 빈 하위 절 3개 대신 헤딩만 남기도록 맞췄다(릴리즈 판정 로직은 불릿 `- ` 유무로 동작하므로 영향 없음).
...
- README '과제 3대 요구사항' 표 가독성 개선: 셀마다 장문·긴 테스트 클래스명을 욱여넣던 4열 표 → 요구사항·한 줄 답·상세 3열 표로 슬림화. 핵심 메커니즘·코드 증명 세부는 이미 존재하는 (2)·(4) 섹션으로 위임(중복 제거).
...
### Changed
- **`docs/project-qna.md` 과감한 재구성** — 리뷰어가 최초로 보는 문서의 가독성 우선. 220줄짜리 "프로젝트 안에 있는 문서들" 메타 카탈로그(각 파일을 언제 여는지 나열)를 7행 표 하나로 축약하고, 중복되던 "AI와 함께 개발한 방식"·"사람 리소스가 덜 드는 이유" 두 절을 짧은 "AI로 개발한 프로젝트" 1개 절(워크플로우 상세는 `docs/ai/`로 위임)로 통합. 본문(구현 포인트·기술 선택·테스트 현황·Q&A 16문항)은 그대로 보존. 634줄 → 374줄.
- **`README.md` 「과제 3대 요구사항」 절 압축** — ①②③ 각 4불릿 블록(약 30줄)을 스캔 가능한 3행 표로 축약. 채점 대상인 「핵심 설계 의사결정 (1)~(4)」는 유지. 210줄 → 190줄.

### Fixed
- **`docs/project-qna.md` 테스트 현황 표의 `CacheInvalidationServiceTest` 개수 13 → 17 정정** — 직전 동기화 커밋(#21)이 총계는 133으로 올렸으나 이 행만 누락해, 표 행 합계(129)가 바로 위 "총 133개"와 어긋나 있었다. 이제 표 합계 = 133으로 일치(CHANGELOG·실제 JUnit 리포트와도 일치).
- **`docs/ai/ai-dev-guide.md`의 "단위 테스트 129개" → "테스트 133개(ArchUnit 규칙 6개 포함)"로 정정** — 동기화된 총계와 모순되던 잔존 표기 제거.
...
### Fixed
- **GitHub Release 노트 끝에 CHANGELOG 버전 구분선(`---`)이 딸려 들어가던 문제 수정** — `release.yml`의 섹션 추출 awk가 `## [X.Y.Z]`부터 다음 `## [` 직전까지를 그대로 긁어, 버전 사이 구분선과 꼬리 빈 줄이 노트 끝에 남았다. 추출 결과의 트레일링 빈 줄·`---` 라인을 제거하도록 보정(노트가 마지막 내용에서 끝남).
...
### Changed
- **README 가독성 개선** — 과밀했던 표 2개(5컬럼 「과제 3대 요구사항 → 충족 방식」, 4컬럼 「(2) 아키텍처 의사결정 R1~R8」)를 가로 스크롤 없이 읽히는 섹션형(소제목 + 불릿)으로 전환. 진짜 데이터성 표(리스크 목록·성능 수치·검증 매핑·Tech Stack)는 표로 유지. 정보 손실 없음.
