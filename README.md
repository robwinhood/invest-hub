...
| Cache | Caffeine (추천 5분 TTL) · 자동 키 버전(`CacheKeyVersionGenerator`) · 무효화·재갱신 API |
...
| **캐시 히트 시 CB·Bulkhead·TL 미소모** | 저실시간 데이터의 자원 점유 최소화 | 추천 Caffeine 5분 TTL |
...
설계가 "그렇게 동작하길 기대한다"가 아니라 **테스트로 강제·회귀 방지**된다. 총 **102개 테스트 전체 통과**(`./gradlew check-all` → `BUILD SUCCESSFUL`).
...
> Task :test              # Kotest + ArchUnit 102개 통과
BUILD SUCCESSFUL
```
...
사용자의 투자 서비스 대시보드를 반환한다. 세 데이터 소스를 가상 스레드에서 병렬로 조회한 후 단일 응답으로 조합한다.
...
|---|---|---|
| `X-User-Id` | ✅ | 사용자 식별자 (운영 환경에서는 API Gateway가 JWT를 검증 후 주입) |

**Response Headers**

| Header | Description |
|---|---|
| `X-Request-Id` | 요청 추적 UUID — 서버 로그의 `requestId` 필드와 1:1 대응 |
...
│  cache/ NoOpCacheEventPublisher · RecommendationCache-    │
│         RefreshStrategy (캐시 보조 인프라)                │
└──────────────────────────────────────────────────────────┘
```
...
| 캐시 | 없음 (POC) | 없음 (실시간 필수) | **Caffeine 5분 TTL** |
...
// → 별도 코드 없이 Caffeine 5분 캐시가 자동 적용됨
@Component
...
**문제**: Redis(영속 캐시)에서 직렬화된 클래스에 필드가 추가/삭제되면, 구 포맷 JSON과 새 클래스가 충돌해 역직렬화 오류가 발생한다. 개발자가 매번 수동으로 캐시 키 버전을 올리면 휴먼 에러가 발생한다.

**해결**: 클래스 구조(프로퍼티 이름+타입)를 SHA-256으로 해시해 캐시 이름에 자동 삽입한다. 구조가 바뀌면 해시도 자동으로 바뀌어 새 키를 사용한다 — 사람 손이 필요 없다.
...
### 10. 캐시 무효화·재갱신 — 범용 인터페이스 기반

ML 모델 교체, 운영 데이터 긴급 수정 시 TTL 만료를 기다리지 않고 즉시 캐시를 비우거나 다시 채울 수 있다. Redis 없이 지금 동작하며, Redis 추가 시 구현체만 교체한다.

```
CacheInvalidationService (오케스트레이터)
  ├─ 로컬 Caffeine 캐시 즉시 evict
  ├─ CacheEventPublisher (port) ──→ NoOpCacheEventPublisher (현재: 로그)
  │                              └→ RedisCacheEventPublisher (향후: Pub/Sub)
  └─ CacheRefreshStrategy (port) ─→ RecommendationCacheRefreshStrategy (즉시 재갱신)
```

| 동작 | 메서드 | 효과 |
|---|---|---|
| 무효화 | `evict(name, key)` | 다음 요청에서 데이터 소스 재조회 |
| 무효화+재갱신 | `evictAndRefresh(name, key)` | 무효화 직후 능동 재조회 → 사용자 캐시 미스 없음 |
| 전체 무효화 | `evictAll(name)` | 캐시 전체 제거 |
...
```
현재 (로컬 캐시만):
  Pod A [Caffeine 5분]  ← 각자 독립
  Pod B [Caffeine 5분]  ← NoOpCacheEventPublisher: 현재 Pod만 무효화
  Pod C [Caffeine 5분]  ←

목표 (Redis Pub/Sub 추가):
  Pod A ─┐  무효화 이벤트 발행
  Pod B ─┼─→ Redis Pub/Sub ──→ 전체 Pod 로컬 캐시 동기화 evict
  Pod C ─┘
```

`CacheEventPublisher` 인터페이스가 이미 준비되어 있어, `RedisCacheEventPublisher`(@Component) 하나만 추가하면 `NoOpCacheEventPublisher`가 자동 비활성화(`@ConditionalOnMissingBean`)되고 분산 무효화가 동작한다. 애플리케이션 코어 코드는 변경 불필요.
...
└── adapter/
    ...
    └── out/
        ...
        └── cache/
            ├── NoOpCacheEventPublisher.kt          # 무효화 이벤트 로그 발행 (Redis 전 기본)
            └── RecommendationCacheRefreshStrategy.kt  # 추천 캐시 재갱신 전략

src/test/kotlin/com/investhub/  (총 102개 테스트)
...
사람은 "비즈니스 타당성 판단 + 최종 승인"에만 리소스를 쓴다.
