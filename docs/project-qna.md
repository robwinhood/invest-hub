...
**한 줄 설명**: `/add-datasource`를 입력했을 때 AI가 따르는 지시서. PRD를 받아서 약 10개 파일을 자동으로 생성하는 방법이 담겨 있다.
...
4. **반복 패턴은 Skill로 자동화한다** — `/add-datasource` skill로 PRD를 입력하면 약 10개 파일이 자동 생성된다.
...
2. (AI)  Claude Code에 "/ship" + 채운 PRD 붙여넣기
         ...
         → PR 본문에 체크리스트 + "수정한 것 / 사람 판단이 필요한 것" 보고서 자동 첨부
3. (사람) PR에서 CI 통과 확인 → 보고서의 "판단 필요 항목"만 검토 → 머지
```

> `/ship`은 **적응형**이라, 코드가 이미 작성된 상태에서 `/ship`만 치면 ④자가검토부터 마무리한다.
> 개별 단계만 쓰고 싶으면 `/add-datasource`(③)·`/self-review`(④)를 따로 실행할 수도 있다.
